...
class Node(pygame.sprite.Sprite):
	...
	def update(self):
		if self.status == 'available':
			...
			tint_suface = self.image.copy()
			tint_suface.fill('black',None,pygame.BLEND_RGBA_MULT)
			self.image.blit(tint_suface,(0,0))
...
class Overworld:
	...
	def draw_paths(self):
		if self.max_level > 0:
			...
			pygame.draw.lines(self.display_surface,'#a04d35',False,points,6)
	...
	def input(self):
		...
		if not self.moving and self.allow_input :
			if keys[pygame.K_RIGHT] and self.current_level<self.max_level: 
				...
				self.move_direction = self.get_movement_data('previese')
				self.current_level -=1
				self.moving = True
